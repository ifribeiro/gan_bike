...
def get_list_dates(data_size=1000, year=None, month=None, day=1, tdeltah=1):
    """
...
def plot_compare(real, falso, expname=""):
    fig, ax = plt.subplots(ncols=2, figsize=(15, 4.5))
    ## soma
    # real
    ano = real.iloc[0].date.year
    mes = real.iloc[0].date.month
    lista_datas = get_list_dates(real.shape[0], year=ano, month=mes)
    df_real = get_df(lista_datas, real[['cnt']].values)
    soma_real = get_count(df_real, 4, timestep=24, column='ts')['cnt_0']

    # falso
    falso = falso.flatten()[:real.shape[0]]
    df_falso = get_df(lista_datas, falso.reshape(-1,1))
    soma_falso = get_count(df_falso, 4, timestep=24, column='ts')['cnt_0']


    ax[0].plot(soma_real, label='real')
    ax[0].plot(soma_falso, label='falso')
    ax[0].legend()

    # CDFs
    sns.kdeplot(real['cnt'], cumulative=True, ax=ax[1], label='real')
    sns.kdeplot(falso, cumulative=True, ax=ax[1], label='fake')
    kl_d = m.kl_divergence(real['cnt'].values, [falso])[0]
    ks_t = m.ks_teste(real['cnt'].values, [falso])[0]
    ax[1].set_title(f"kl_d: {np.round(kl_d, decimals=2)} | ks_t: {ks_t}")

    ax[1].legend()
    plt.suptitle(f"{expname}")
